...
from utils import *
from xtts import webui as xtts
...
def generate_proxy(
    ...
    if not os.path.exists(rvc_config_path):
        raise gr.Error(
            "No RVC configuration found, check configs folder.  If rvc.json does not exist, please change a setting "
            "in the RVC area to create one.")
    kwargs = locals()
    kwargs['xtts_active'] = XTTS_ACTIVE
...
def read_generate_settings_proxy(file, save_as='.temp'):
    j, latents = read_generate_settings(file)

    if latents:
        outdir = f'{get_voice_dir()}/{save_as}/'
...
def transcribe_other_language_proxy(voice, language, chunk_size, continuation_directory, align, rename, num_processes,
    ...
    for item in items_to_move:
        if os.path.exists(os.path.join(training_folder, os.path.basename(item))):
            raise gr.Error(
                f'Remove ~~train.txt ~~validation.txt ~~audio(folder) from "./training/{voice}" before trying to '
                f'transcribe a new dataset. Or click the "Archive Existing" button')
    ...
    try:
        ...
        raise gr.Error(
            'No files found in the voice folder specified, make sure it is not empty.  If you interrupted the '
            'process, the files may be in the "original_files" folder')
...
def get_valid_rvc_folder():
    if type(RVC_SETTINGS['use_rvc_install']) is gr.Checkbox:
        if not RVC_SETTINGS['use_rvc_install'].value:
            return None
    elif not RVC_SETTINGS['use_rvc_install']:
        return None

    if type(RVC_SETTINGS['rvc_folder']) is gr.Textbox:
        folder = RVC_SETTINGS['rvc_folder'].value
    else:
        folder = RVC_SETTINGS['rvc_folder']

    weights = os.path.exists(os.path.join(folder, "assets", "weights"))
    logs = os.path.exists(os.path.join(folder, "logs"))
    if weights and logs:
        return folder
    else:
        gr.Warning(f"Could not find valid rvc folder: {folder}")
        return None


def update_voices(voice):
    rvc_folder = get_valid_rvc_folder()
    return (
        ...
        gr.Dropdown(choices=get_rvc_models(rvc_folder)),  # Update for RVC models
        gr.Dropdown(choices=get_rvc_indexes(rvc_folder)),  # Update for RVC models
        gr.Dropdown(choices=get_voice_list()),
...
def setup_gradio():
    ...
    with gr.Blocks() as ui:
        with gr.Tab("Generate"):
            ...
            with gr.Row():
                settings = import_generate_settings()
                XTTS_ACTIVE = settings['xtts_active'] if 'xtts_active' in settings else False
                with gr.Tabs(selected=int(XTTS_ACTIVE)):
                    if args.tts_backend == "tortoise":
                        ...
                        tab_name = "Vall-E"
                    with gr.Tab(tab_name, id=0) as tab:
                        tortoise_tab = tab
                        with gr.Row():
                            with gr.Column():
                                ...
                                # it'd be very cash money if gradio was able to default to the first value in the list
                                # without this shit
                                GENERATE_SETTINGS["voice"] = gr.Dropdown(
                                    choices=voice_list_with_defaults, label="Voice",
                                    type="value", value=voice_list_with_defaults[0])
                    ...
                    def set_xtts(selected):
                        global XTTS_ACTIVE
                        XTTS_ACTIVE = selected.label == "XTTS"

                    tortoise_tab.select(lambda: set_xtts(tortoise_tab))
                    with gr.Tab("XTTS", id=1) as tab:
                        xtts_tab = tab
                        xtts.ui()
                    xtts_tab.select(lambda: set_xtts(xtts_tab))
                with gr.Column():
                    rvc_folder = get_valid_rvc_folder()
                    EXEC_SETTINGS['use_rvc'] = gr.Checkbox(label="Run the outputted audio through RVC",
                                                           value=args.use_rvc)
                    RVC_SETTINGS['use_rvc_install'] = gr.Checkbox(label="Load RVC models from a local RVC install",
                                                                  value=RVC_SETTINGS['use_rvc_install'])
                    RVC_SETTINGS['rvc_folder'] = gr.Textbox(label="Install folder",
                                                            placeholder="D:\\RVC1006Nvidia",
                                                            value=RVC_SETTINGS['rvc_folder'],
                                                            visible=RVC_SETTINGS['use_rvc_install'].value)
                    RVC_SETTINGS['rvc_model'] = gr.Dropdown(choices=get_rvc_models(rvc_folder), label="RVC Voice Model",
                                                            value=RVC_SETTINGS['rvc_model'], interactive=True)
                    RVC_SETTINGS['file_index'] = gr.Dropdown(choices=get_rvc_indexes(rvc_folder),
                                                             label="RVC Index File",
                                                             value=RVC_SETTINGS["file_index"], interactive=True)
                    RVC_SETTINGS['index_rate'] = gr.Slider(minimum=0, maximum=1, label="Index Rate",
                                                           value=RVC_SETTINGS["index_rate"], interactive=True)
                    RVC_SETTINGS['f0_up_key'] = gr.Slider(minimum=-24, maximum=24, label="Voice Pitch (f0 key)",
                                                          value=RVC_SETTINGS["f0_up_key"], interactive=True)
                    # RVC_SETTINGS['f0_method'] = gr.Dropdown(choices=get_rvc_models(), label="RVC Voice Model", value=args.rvc_model)
                    RVC_SETTINGS['filter_radius'] = gr.Slider(minimum=0, maximum=7, label="Filter Radius",
                                                              value=RVC_SETTINGS["filter_radius"], interactive=True)
                    RVC_SETTINGS['resample_sr'] = gr.Slider(minimum=0, maximum=48000, label="Resample sample rate",
                                                            value=RVC_SETTINGS["resample_sr"], interactive=True)
                    RVC_SETTINGS['rms_mix_rate'] = gr.Slider(minimum=0, maximum=1,
                                                             label="RMS Mix Rate (Volume Envelope)",
                                                             value=RVC_SETTINGS["rms_mix_rate"], interactive=True)
                    RVC_SETTINGS['protect'] = gr.Slider(minimum=0, maximum=0.5,
                                                        label="Protect Voiceless Consonants",
                                                        value=RVC_SETTINGS["protect"], interactive=True)
                    with gr.Column():
                        with gr.Row():
            ...
            with gr.Tab("Prepare Dataset for Large Files"):
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        # dataset2_settings = list(dataset2_settings.values()) # Really only need this for tqdm to
                        # extract values
        ...
        with gr.Tab("Settings"):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    # EXEC_SETTINGS['tts_backend'] = gr.Dropdown(TTSES, label="TTS Backend", value=args.tts_backend
                    # if args.tts_backend else TTSES[0])
                ...
                with gr.Column(visible=args.tts_backend == "tortoise"):
                    ...
                    def update_model_list_proxy(autoregressive, diffusion, tokenizer):
                        autoregressives = get_autoregressive_models()
                        if autoregressive not in autoregressives:
                            autoregressive = autoregressives[0]

                        diffusions = get_diffusion_models()
                        if diffusion not in diffusions:
                            diffusion = diffusions[0]

                        tokenizers = get_tokenizer_jsons()
                        if tokenizer not in tokenizers:
                            tokenizer = tokenizers[0]

                        return (
                            gr.update(choices=autoregressives,
                                      value=autoregressive),
                            gr.update(choices=diffusions,
                                      value=diffusion),
                            gr.update(choices=tokenizers,
                                      value=tokenizer),
                        )
        ...
        RVC_SETTINGS['use_rvc_install'].change(
            fn=lambda x: gr.update(visible=x),
            inputs=RVC_SETTINGS['use_rvc_install'],
            outputs=RVC_SETTINGS['rvc_folder']
        )
        ...
        transcribe_button.click(
            ...
            inputs=list(dataset_settings.values()),
        ...
        transcribe_all_button.click(
            ...
            inputs=list(dataset_settings.values())[1:],
            outputs=prepare_dataset_output  # console_output
        )
        diarize_button.click(
            diarize_dataset,
            inputs=list(dataset_settings.values())[0],
            outputs=prepare_dataset_output  # console_output
        )
