...
from xtts.utils import voice_preview
...
def generate(**kwargs):
    if kwargs["xtts_active"]:
        return generate_xtts(**kwargs)
...
def generate_tortoise(**kwargs):
    ...
    def get_info(voice, settings=None, latents=True):
        ...
        if latents and "latents" not in info:
            voice = info['voice']
            model_hash = settings["model_hash"][:8]\
                if settings is not None and "model_hash" in settings else tts.autoregressive_model_hash[:8]
    ...
    for line, cut_text in enumerate(texts):
        ...
        print(settings)
        if progress is not None:
            notify_progress("Generating line...", progress=progress)
        try:
            if args.use_hifigan:
                ...
                unused_args = ['diffusion_temperature', 'cond_free_k', 'sample_batch_size', 'diffusion_iterations',
                               ...
                               'cond_free', 'autoregressive_model', 'diffusion_model', 'tokenizer_json']
...
def generate_xtts(**kwargs):
    info = {}
    info.update(kwargs)
    progress = info['progress'] if 'progress' in info else None
    info['progress'] = None
    del info['progress']
    start_time = time.time()
    outdir = os.path.join(args.results_folder, "xtts")
    os.makedirs(outdir, exist_ok=True)

    def progress_function(text):
        notify_progress(text, progress=progress)

    output_voices = voice_preview(info, outdir, progress_function if progress is not None else None)
    if type(output_voices) is list:
        if args.use_rvc:
            if progress is not None:
                notify_progress(f"Running through RVC...", progress=progress)
            generate_rvc(output_voices)
        info['time'] = time.time() - start_time
        print(f"Generation took {info['time']} seconds, saved to '{output_voices[0]}'\n")
    else:
        info['time'] = time.time() - start_time
        gr.Warning(output_voices['error'])
        output_voices = []
...
def generate_rvc(output_voices: list[str]):
    rvc_settings = load_rvc_settings()
    if rvc_settings['use_rvc_install']:
        rvc_model_path = os.path.join(rvc_settings['rvc_folder'], "assets", "weights", rvc_settings['rvc_model'])
        rvc_index_path = os.path.join(rvc_settings['rvc_folder'], "logs", rvc_settings['file_index'])
    else:
...
# Stuff added by Jarod changed by Justoboy
def get_rvc_models(folder_path=None):
    if folder_path is None:
        folder_path = 'models/rvc_models'
    else:
        folder_path = os.path.join(folder_path, "assets\\weights")
    return [f for f in os.listdir(folder_path)
            if os.path.isfile(os.path.join(folder_path, f)) and f.endswith('.pth')]


def get_rvc_indexes(folder_path=None):
    if folder_path is None:
        folder_path = 'models/rvc_models'
        return [f for f in os.listdir(folder_path) if
                os.path.isfile(os.path.join(folder_path, f)) and f.endswith('.index')]
    else:
        ...
        for root, dirs, files in os.walk(folder_path, topdown=False):
            for name in files:
                if name.endswith(".index") and "trained" not in name:
                    index_path = os.path.relpath(os.path.join(root, name), folder_path)
